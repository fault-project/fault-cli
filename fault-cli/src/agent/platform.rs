use std::fmt;
use std::fs;
use std::sync::Arc;

use anyhow::Result;
use chrono::Utc;
use kanal::AsyncSender;
use pulldown_cmark::Event;
use pulldown_cmark::HeadingLevel;
use pulldown_cmark::Options;
use pulldown_cmark::Parser;
use pulldown_cmark::Tag;
use pulldown_cmark::TagEnd;
use pulldown_cmark_to_cmark::cmark;
use serde::Deserialize;
use serde::Serialize;
use swiftide::indexing::EmbeddedField;
use swiftide::integrations::lancedb::LanceDB;
use swiftide::query;
use swiftide::query::answers;
use swiftide::query::query_transformers;
use swiftide_core::EmbeddingModel;
use swiftide_core::SimplePrompt;
use tera::Context;

use super::CODE_COLLECTION;
use super::clients::SupportedLLMClient;
use super::clients::get_client;
use crate::discovery::types::Resource;
use crate::discovery::types::ResourcePlatform;
pub async fn analyze(
    platform_type: ResourcePlatform,
    resource: Resource,
    role: &PlatformReviewRole,
    sender: AsyncSender<PlatformReviewEvent>,
    client_type: SupportedLLMClient,
    prompt_model: &str,
    embed_model: &str,
    embed_model_dimension: u64,
) -> Result<PlatformReviews> {
    let mut advices = Vec::new();

    let llm = get_client(client_type, prompt_model, embed_model)?;

    // upcast for later calls
    let sp: Arc<dyn SimplePrompt> = llm.clone();
    let em: Arc<dyn EmbeddingModel> = llm.clone();

    let lancedb: LanceDB = LanceDB::builder()
        .uri(crate::agent::LANCEDB_URI)
        .table_name(CODE_COLLECTION)
        .batch_size(50usize)
        .vector_size(embed_model_dimension as i32)
        .with_vector(EmbeddedField::Combined)
        .build()?;

    // we carry a multiple shots analysis for better advices
    // each shot response is injected into the next shot
    let shots = [
        PlatformReviewEventPhase::Summary,
        PlatformReviewEventPhase::Deepdive,
        PlatformReviewEventPhase::Recommendations,
        PlatformReviewEventPhase::Risks,
        PlatformReviewEventPhase::Executive,
    ];

    let r_yaml = serde_yaml::to_string(&resource)?;

    let mut previous = String::new();
    for phase in shots.iter() {
        sender
            .send(PlatformReviewEvent {
                phase: phase.clone(),
                advice: previous.clone(),
            })
            .await?;

        let mut ctx = Context::new();
        ctx.insert("role", &role.to_string());
        ctx.insert("resource", &r_yaml);
        if !previous.is_empty() {
            ctx.insert("previous_advice", &previous);
        }
        let template = match phase {
            PlatformReviewEventPhase::Summary => match platform_type {
                ResourcePlatform::Kubernetes => {
                    include_str!("prompts/platform-k8s-summary.md")
                }
                ResourcePlatform::Gcp => {
                    include_str!("prompts/platform-gcp-summary.md")
                }
            },
            PlatformReviewEventPhase::Deepdive => {
                include_str!("prompts/platform-resource-deepdive.md")
            }
            PlatformReviewEventPhase::Recommendations => {
                include_str!("prompts/platform-recommendations.md")
            }
            PlatformReviewEventPhase::Risks => {
                include_str!("prompts/platform-risks.md")
            }
            PlatformReviewEventPhase::Executive => {
                include_str!("prompts/platform-executive.md")
            }
            other => return Err(anyhow::anyhow!("Unknown shot: {}", other)),
        };
        let q: String = tera::Tera::one_off(template, &ctx, false)?;

        let pipeline = query::Pipeline::default()
            .then_transform_query(
                query_transformers::GenerateSubquestions::from_client(
                    sp.clone(),
                ),
            )
            .then_transform_query(query_transformers::Embed::from_client(
                em.clone(),
            ))
            .then_retrieve(lancedb.clone())
            .then_answer(answers::Simple::from_client(sp.clone()));

        let resp = pipeline.query(q).await?;
        let advice = normalize_markdown(&resp.answer().to_string())?;
        advices.push(PlatformAdvice {
            role: role.clone(),
            phase: phase.clone(),
            advice: advice.clone(),
        });
        previous = advice;
    }

    sender
        .send(PlatformReviewEvent {
            phase: PlatformReviewEventPhase::Completed,
            advice: "".to_string(),
        })
        .await?;

    Ok(PlatformReviews { advices })
}

#[derive(Debug, Clone)]
pub struct PlatformAdvice {
    pub role: PlatformReviewRole,
    pub phase: PlatformReviewEventPhase,
    pub advice: String,
}

#[derive(Debug, Clone)]
pub struct PlatformReviews {
    pub advices: Vec<PlatformAdvice>,
}

impl PlatformReviews {
    pub fn save(&self, path: &str) -> Result<String> {
        let md = self.stitch()?;
        fs::write(path, md.clone())?;
        Ok(md)
    }

    pub fn stitch(&self) -> Result<String> {
        let mut stitched = String::new();
        let mut executive = String::new();

        for advice in self.advices.iter() {
            if advice.phase == PlatformReviewEventPhase::Executive {
                executive.push_str(&advice.advice);
                continue;
            }

            stitched.push_str("\n\n");
            stitched.push_str(&advice.advice);
        }

        let mut md = String::new();
        md.push_str(&format!("# Platform resilience report analysis"));

        md.push_str("\n\n");
        let toc = generate_toc(&stitched);
        md.push_str(&toc);
        md.push_str("\n\n---\n\n");
        md.push_str(&executive);
        md.push_str("\n\n");
        md.push_str(&stitched);
        md.push_str("\n\n---\n\n");
        md.push_str(&format!("Generated on {}\n\n", Utc::now().to_string()));

        Ok(md)
    }
}

#[derive(
    clap::ValueEnum, Clone, Debug, Serialize, Deserialize, Eq, PartialEq,
)]
#[serde(rename_all = "lowercase")]
pub enum PlatformReviewRole {
    Developer,
    Sre,
}

impl fmt::Display for PlatformReviewRole {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PlatformReviewRole::Developer => write!(f, "Developer"),
            PlatformReviewRole::Sre => write!(f, "SRE"),
        }
    }
}

impl PlatformReviewRole {
    pub fn from_str(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
            "developer" => Some(PlatformReviewRole::Developer),
            "sre" => Some(PlatformReviewRole::Sre),
            _ => None,
        }
    }
}

impl Default for PlatformReviewRole {
    fn default() -> Self {
        Self::Developer
    }
}

fn generate_toc(input: &str) -> String {
    let opts = Options::all();
    let parser: Parser<'_> = Parser::new_ext(input, opts);

    // Collect (level, text) for each heading
    let mut headings = Vec::new();
    let mut in_header = false;
    let mut lvl = 0;
    let mut buf = String::new();

    // locate all H2/H3 to build a TOC
    for ev in parser {
        match ev {
            Event::Start(tag) => match tag {
                Tag::Heading { level, .. }
                    if (level == HeadingLevel::H2
                        || level == HeadingLevel::H3) =>
                {
                    in_header = true;
                    lvl = level as usize;
                    buf.clear();
                }
                _ => {}
            },
            Event::End(tag) => match tag {
                TagEnd::Heading(level, ..)
                    if (level == HeadingLevel::H2
                        || level == HeadingLevel::H3) =>
                {
                    in_header = false;
                    headings.push((lvl, buf.clone()));
                }
                _ => {}
            },
            Event::Text(t) if in_header => buf.push_str(&t),
            _ => {}
        }
    }

    // Build the TOC string
    let mut toc = String::from("## Table of Contents\n\n");
    for (level, text) in headings {
        let indent = "  ".repeat(level - 2);
        let anchor = text
            .to_lowercase()
            .chars()
            .filter(|c| c.is_alphanumeric() || c.is_whitespace() || *c == '-')
            .map(|c| if c.is_whitespace() { '-' } else { c })
            .collect::<String>();
        toc.push_str(&format!("{indent}- [{text}](#{anchor})\n"));
    }
    toc.push('\n');
    toc
}

fn normalize_markdown(input: &str) -> Result<String> {
    let opts = Options::all();
    let parser: Parser<'_> = Parser::new_ext(&input, opts);

    let mut buf = String::with_capacity(input.len() + 1024);
    cmark(parser, &mut buf)?;

    Ok(buf)
}

#[derive(Debug, Clone, PartialEq)]
pub enum PlatformReviewEventPhase {
    Summary,
    Deepdive,
    Recommendations,
    Risks,
    Executive,
    Completed,
}

impl fmt::Display for PlatformReviewEventPhase {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PlatformReviewEventPhase::Summary => write!(f, "summary"),
            PlatformReviewEventPhase::Deepdive => write!(f, "deepdive"),
            PlatformReviewEventPhase::Recommendations => {
                write!(f, "recommendations")
            }
            PlatformReviewEventPhase::Risks => write!(f, "risks"),
            PlatformReviewEventPhase::Executive => write!(f, "executive"),
            PlatformReviewEventPhase::Completed => write!(f, "completed"),
        }
    }
}

impl PlatformReviewEventPhase {
    pub fn long_form(&self) -> String {
        match self {
            PlatformReviewEventPhase::Summary => {
                "Building platform summary".to_string()
            }
            PlatformReviewEventPhase::Deepdive => {
                "Exploring each platform resource".to_string()
            }
            PlatformReviewEventPhase::Recommendations => {
                "Crafting actionable recommendations".to_string()
            }
            PlatformReviewEventPhase::Risks => "Deriving risks".to_string(),
            PlatformReviewEventPhase::Executive => {
                "Creating executive summary".to_string()
            }
            PlatformReviewEventPhase::Completed => {
                "Completed analysis".to_string()
            }
        }
    }
}

#[derive(Debug, Clone)]
pub struct PlatformReviewEvent {
    pub phase: PlatformReviewEventPhase,
    pub advice: String,
}
